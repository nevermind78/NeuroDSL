...
We compare the forward+backward time and peak GPU memory on a standard Transformer block (RMSNorm, single‑head attention, SwiGLU MLP) with sequence length 64 and varying hidden dimensions. For PyTorch, we evaluate three configurations:
\begin{itemize}
    \item \textbf{PyTorch eager} – standard eager execution,
    \item \textbf{PyTorch + checkpoint} – gradient checkpointing with segments of 2 layers,
    \item \textbf{NeuroDSL} – incremental mode with full memory planning.
\end{itemize}
Timings are medians over 50 samples after warm‑up; memory is measured with \texttt{MemTrack} (Julia) and \texttt{torch.cuda.max\_memory\_allocated} (PyTorch).
...
\toprule
& \multicolumn{4}{c}{\textbf{Time (ms)}} & \multicolumn{3}{c}{\textbf{Peak Memory (MiB)}} \\
\cmidrule(lr){2-5} \cmidrule(l){6-8}
\textbf{dim} & \textbf{Flux} & \textbf{ND} & \textbf{PyT Eager} & \textbf{PyT Ckpt} & \textbf{ND} & \textbf{PyT Eager} & \textbf{PyT Ckpt} \\
\midrule
256  & 8.18 & 6.55 & 5.10 & 5.48 & 3.47  & 20.32 & 14.85 \\
512  & 8.42 & 6.09 & 5.14 & 5.72 & 10.35 & 31.38 & 22.10 \\
1024 & 8.44 & 5.96 & 5.77 & 6.33 & 34.61 & 74.51 & 52.17 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Memory savings beyond checkpointing.} Even with gradient checkpointing, PyTorch retains the inputs of each checkpointed segment. NeuroDSL’s interval‑coloring planner reuses buffers across the whole block, including those holding checkpointed activations. This yields a further $\sim$30\% reduction in peak memory compared to PyTorch+checkpoint, as shown in the last two columns. The time overhead of NeuroDSL remains competitive, within 3–5\% of PyTorch eager.

\paragraph{Note on small‑scale measurements.} At these modest dimensions (tens of MiB), a fraction of PyTorch’s reported memory is fixed CUDA context overhead. The relative advantage of NeuroDSL becomes even more pronounced at larger scales (see Section~\ref{sec:scalability}).
...
\paragraph{Incremental backward.} \Cref{tab:incr_backward} shows the median time over 50 runs for a full backward pass versus an incremental one that only recomputes the gradients affected by the changed parameter, on two configurations. Results range from a 24.4\% reduction (10-layer MLP, last parameter) to a 43.8\% reduction (8-layer MLP, sparse fine-tuning of the last layer); the exact gain is workload-dependent, as discussed below.

\begin{table}[H]
\centering
\caption{Incremental backward validation on deep MLPs.}
...
\cmidrule{1-3}
10-layer MLP (dim=64) full recomputation & 44\,943.8 $\mu$s & baseline \\
10-layer MLP (dim=64) incremental (last param) & 33\,999.6 $\mu$s & 24.4\% faster \\
\rowcolor{rowgray}
\raggedright 8-layer MLP ($d=512$, $L=8$) Sparse Fine-Tuning (Layer 8) & $19.66 \pm 9.27\text{ ms} \to 11.05 \pm 5.17\text{ ms}$ & \textbf{43.8\% speedup} \\
\midrule
\multicolumn{3}{c}{\textit{Gradient drift validation}} \\
\cmidrule{1-3}
12-layer MLP (dim=512, RMSNorm) & relative error $=$ 0.0 & full vs.\ sparse backward \\
\bottomrule
\end{tabularx}
\end{table}
As reported in Table~\ref{tab:incr_backward}, the framework achieves a \textbf{43.8\% performance gain} during the fine-tuning backward phase, dropping from 19.66~ms in full mode to 11.05~ms in sparse mode. Although a higher variance ($\pm$9.27~ms) is observed during the initial un-warmed stages due to Julia's dynamic memory tracking overhead, the execution latency stabilizes significantly ($\pm$5.17~ms) once the warm-up cycles are completed. This empirical result strongly confirms the practical viability of our localized $\mathcal{O}(|\mathcal{V}_{\theta}|)$ graph traversals.

\paragraph{Gradient drift validation.}
We verified the numerical exactness of the incremental backward on a 12‑layer MLP (dim=512, with RMSNorm). After a full backward pass, we simulated a partial invalidation of the last layer and ran \texttt{backward\_graph!} with \texttt{full=false}. The gradients of all parameters of the last layer matched the full‑backward gradients with a relative error of \textbf{0.0} (within machine precision), confirming Theorem~2 in practice.

\paragraph{Speed is not yet a general guarantee.} What is solidly established, across multiple graph topologies (sequential, branching, residual, multiple frozen layers), is the numerical exactness result above. The \emph{speed} gain, however, is workload-dependent: it follows from the impact-subgraph bound of \Cref{sec:invalidation_proof} when the modified parameter's impact subgraph is a small suffix of the network, but can be dominated by fixed dispatch overhead elsewhere. On the larger, more realistic architecture of \Cref{sec:scalability}, for instance, the same sparse-backward mechanism measured \emph{slower} wall-clock time than a full backward pass (497.1~ms full vs.\ 897.0~ms sparse with 10/12 layers frozen), even though the returned gradients remained numerically exact. We report the specific configurations above rather than a general range, since we do not yet have a benchmark suite that reliably predicts when the speed gain holds.
...
Peak GPU memory (dim=1024)   & 34.61 MiB & 2.15$\times$ less than PyTorch eager \\
Iris test accuracy           & 100\%     & -- \\
Fusion speedup (simple chain) & 37--54\%  & -- \\
Incremental backward gain (8-layer MLP, last layer) & 43.8\% & vs full backward \\
Incremental gradient drift (12-layer MLP) & 0.0 (machine precision) & vs full backward \\
...
GPT-2-shaped model, sparse-backward wall-clock & 80.4\% slower & counterexample, see \Cref{sec:scalability} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Gradient correctness at scale.} The incremental backward has been validated as numerically exact on up to 12‑layer MLPs and on a 12-layer GPT-2-shaped toy model (\Cref{sec:scalability}). A formal proof of gradient exactness is provided in Theorem~2. Numerical validation on real-scale Transformer models (hundreds of millions of parameters, real corpora, GPU execution) is planned for future work, as is closing the wall-clock overhead gap identified in \Cref{sec:scalability}.
