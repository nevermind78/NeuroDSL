...
\begin{enumerate}
    ...
    \item \textbf{Interactive visualization} -- a real-time viewer showing forward (blue) and backward (red) passes, inspired by tools like Netron \cite{netron} but fully dynamic.
\end{enumerate}
...
\label{fig:dag}
\end{figure}
The architectural core of \textsc{NeuroDSL} is its reactive computational model, which enables localized updates without the overhead of global graph recompilation. As illustrated in Figure 2, unlike traditional frameworks that trigger a full rebuild upon topological changes, \textsc{NeuroDSL} confines invalidation to the immediate neighborhood of the modified parameter, ensuring efficient local evolution.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[
    % On augmente la distance horizontale à 3cm pour écarter Graph A et B
    ...
    % On place l'étiquette en dessous du milieu du segment pour qu'elle ne soit plus coincée
    ...
    % La flèche rouge passe au-dessus (bend right = courbure vers le haut)
...
To further reduce computation when only part of the graph changes, \textsc{NeuroDSL} introduces \texttt{\_invalidate\_upstream!}. When a parameter is modified, this function clears the gradients of all nodes that depend on it, while leaving other gradients intact. The subsequent \texttt{backward\_graph!} then only recomputes the affected gradients, yielding significant savings (see \Cref{tab:incr_backward}).
...
\subsection{Formal Proof of Invalidation Complexity and Namespace Isolation}
A potential concern in dynamic frameworks handling very large DAGs ($|V| \to \infty$) is the computational overhead on the CPU during the graph traversal phase of \texttt{\_invalidate\_upstream!}. If the traversal covers the full unrolled graph, the CPU hashing and key lookup costs could offset the GPU execution gains. We provide a formal proof demonstrating how NeuroDSL's structural \emph{Namespaces} isolate mutations and guarantee a strict constant bound on complexity.

\paragraph{Mathematical Formulation.}
Let the global execution graph be a hierarchical DAG denoted by $\mathcal{G} = (\mathcal{V}, \mathcal{E})$. NeuroDSL partitions the complete vertex set $\mathcal{V}$ into a finite set of $K$ disjoint subsets representing distinct architectural namespaces (e.g., individual Transformer layers):
\begin{equation}
    \mathcal{V} = \bigsqcup_{k=1}^{K} \mathcal{N}_k \quad \text{where } \mathcal{N}_i \cap \mathcal{N}_j = \emptyset \quad \forall i \neq j
\end{equation}
Let $v_{i, \alpha} \in \mathcal{N}_i$ represent the node $\alpha$ belonging to namespace $\mathcal{N}_i$. The total edge set $\mathcal{E}$ is naturally divided into intra-namespace edges $\mathcal{E}_{\text{intra}}$ and inter-namespace edges $\mathcal{E}_{\text{inter}}$:
\begin{equation}
    \mathcal{E}_{\text{intra}} = \{ (u, v) \in \mathcal{E} \mid \exists k \text{ s.t. } u, v \in \mathcal{N}_k \}
\end{equation}
\begin{equation}
    \mathcal{E}_{\text{inter}} = \{ (u, v) \in \mathcal{E} \mid u \in \mathcal{N}_i, v \in \mathcal{N}_j, i \neq j \}
\end{equation}
In standard Deep Transformer architectures, inter-namespace connections are highly sparse and strictly directed, connecting consecutive layers exclusively through boundary residual connections. Thus, the interface degree is strictly bounded by a small constant $c_{\text{inter}}$:
\begin{equation}
    |\{ (u, v) \in \mathcal{E}_{\text{inter}} \mid u \in \mathcal{N}_i, v \in \mathcal{N}_{i+1} \}| \le c_{\text{inter}}
\end{equation}

\paragraph{Theorem (Constant Time Invalidation Bound).}
\textit{Let a deep neural model consist of $K$ sequential layers, each encapsulated in a unique NeuroDSL Namespace $\mathcal{N}_k$ containing at most $M$ atomic nodes ($|\mathcal{N}_k| \le M$). The time complexity of executing \texttt{\_invalidate\_upstream!} upon the mutation of a local parameter $\theta \in \mathcal{N}_k$ is strictly bounded by $\mathcal{O}(M)$, making it invariant to the total depth or total node count of the global network as $K \to \infty$.}

\paragraph{Proof.}
Let $\text{Anc}(\theta)$ represent the set of all ancestor vertices visited by the breath-first traversal of \texttt{\_invalidate\_upstream!} on the transposed graph $\mathcal{G}^T$, starting from the updated parameter node $\theta$.

NeuroDSL's reactive engine evaluates a conditional traversal pruning predicate. For any node $u$ encountered during the backward propagation of the invalidation wave:
\begin{equation}
    \text{If } u \in \mathcal{N}_i \land \text{is\_param}(u) = \text{false} \land u \notin \text{Path}(\theta) \implies \text{PruneTraversal}(u)
\end{equation}

We analyze the traversal in two successive structural phases:
\begin{enumerate}
    \item \textbf{Intra-Namespace Phase:} The traversal originates at $\theta \in \mathcal{N}_k$. The wave moves backward through the local nodes of the same sub-graph. The number of visited local nodes is trivially upper-bounded by the total cardinality of that specific namespace:
    \begin{equation}
        |\text{Anc}(\theta) \cap \mathcal{N}_k| \le |\mathcal{N}_k| \le M
    \end{equation}
    \item \textbf{Inter-Namespace Interface Phase:} When the traversal path attempts to cross an inter-namespace boundary edge back into the preceding layer's namespace $\mathcal{N}_{k-1}$, it hits the boundary activation nodes of layer $k-1$. These nodes act as forward inputs to $\mathcal{N}_k$. By definition:
    \begin{itemize}
        \item These boundary nodes are intermediate activations, meaning $\text{is\_param}(u) = \text{false}$.
        \item They reside in a namespace whose parameters are static during this local optimization step, implying they do not belong to the mathematical dependency path $\text{Path}(\theta)$ triggered by the mutation of $\theta$.
    \end{itemize}
    Consequently, the pruning predicate evaluates to true for all boundary entries of $\mathcal{N}_{k-1}$. The invalidation wave is completely blocked at the namespace interface. No internal edges or nodes of $\mathcal{N}_{k-1}, \mathcal{N}_{k-2}, \dots, \mathcal{N}_1$ are ever inserted into the traversal queue.
\end{enumerate}
Thus, the active set of visited vertices is strictly localized within the boundaries of the mutated namespace:
\begin{equation}
    \text{Anc}(\theta) \subseteq \mathcal{N}_k \implies |\text{Anc}(\theta)| \le M
\end{equation}
Since the graph is sparse ($|E| \propto |V|$ inside the namespace), the time complexity of the traversal is directly proportional to the size of the visited subset:
\begin{equation}
    \text{Time}(\text{\_invalidate\_upstream!}) = \mathcal{O}(|\text{Anc}(\theta)| + |\mathcal{E}_{\text{visited}}|) \le \mathcal{O}(M + c \cdot M) = \mathcal{O}(M)
\end{equation}
This completes the proof. By mapping the network into isolated structural Namespaces, NeuroDSL completely neutralizes the CPU graph-overhead problem, enabling surgical $\mathcal{O}(M)$ execution tracking even on infinitely deep networks. \hfill $\blacksquare$

\subsection{Automatic Operator Fusion}
\textsc{NeuroDSL} can rewrite chains of rules into a single fused rule, eliminating intermediate memory allocations and kernel launches. The fusion engine uses a pattern table (\texttt{FUSION\_TABLE}) and is triggered by the user with \texttt{\_fuse!(g, chain)}. Currently, \texttt{matmul+relu} and \texttt{relu+matmul} are supported, providing a \textbf{37\% speedup} on CPU (and up to 54\% on GPU). The framework is designed to easily add new patterns, akin to the approach used in TVM \cite{tvm}.
...
In this section, we establish the formal mathematical foundations of \textsc{NeuroDSL}. The behavior of the dynamic reactive computational graph, automated differentiation, and the memory scheduler are rigorously modeled using algebraic structures, graph homomorphisms, and partially ordered sets (posets).

\subsection{Complexity of Hierarchical Invalidation via Structural Namespaces}

Let the global computational graph be modeled as a hierarchical Directed Acyclic Graph (DAG) $\mathcal{G} = (\mathcal{V}, \mathcal{E})$. We partition the vertex set $\mathcal{V}$ into a finite collection of $K$ disjoint subsets representing the structural execution namespaces:
\begin{equation}
\mathcal{V} = \bigsqcup_{k=1}^{K} \mathcal{N}_k \quad \text{such that } \mathcal{N}_i \cap \mathcal{N}_j = \emptyset \quad \forall i \neq j
\end{equation}

The edge set $\mathcal{E}$ is divided into intra-namespace edges $\mathcal{E}_{\text{intra}}$ and inter-namespace edges $\mathcal{E}_{\text{inter}}$. In deep Transformer architectures, inter-namespace connections linking consecutive layers are decoupled at well-defined structural boundaries via residual stream variables.

\begin{theorem}[Namespace-Bounded Local Complexity]
Consider a network architecture composed of $K$ sequential blocks, each encapsulated within a single namespace $\mathcal{N}_k$ containing at most $M$ atomic nodes ($|\mathcal{N}_k| \le M$). Under the strict operational assumption of parameter non-sharing (the absence of cross-boundary weight-tying mechanisms) and isolated local functional expressions, the time complexity of executing the upstream invalidation algorithm \texttt{\_invalidate\_upstream!} upon the mutation of a local parameter $\theta \in \mathcal{N}_k$ is upper-bounded by $\mathcal{O}(M)$, rendering it strictly invariant with respect to the global depth of the architecture ($K \to \infty$).
\end{theorem}

\begin{proof}
Let $\text{Anc}(\theta)$ denote the set of ancestor vertices visited by the breadth-first traversal of \texttt{\_invalidate\_upstream!} operating on the transposed graph $\mathcal{G}^T$, initialized at the modified parameter node $\theta$. By design of the network interface, pruning predicates evaluate the structural properties of nodes locally.

Upstream invalidation propagates backward through the computational dependency lines. Since the downstream namespaces $\mathcal{N}_{k+1}, \dots, \mathcal{N}_K$ are located exclusively ahead of $\mathcal{N}_k$ in the global topological order, no functional dependency edges originate from these blocks toward the local parameters of $\mathcal{N}_k$. Under the parameter isolation and non-sharing constraint, no external weights introduce implicit backward linkages. Consequently, the set of visited ancestors is strictly confined within the local namespace boundaries:
\begin{equation}
\text{Anc}(\theta) \subseteq \mathcal{N}_k
\end{equation}
Because the search space cardinality is bounded by $|\mathcal{N}_k| \le M$, and every flag lookup or modification operates in $\mathcal{O}(1)$ time via Julia's internal symbol hash tables, the total CPU overhead of upstream invalidation is bounded by $\mathcal{O}(M)$, completely independent of the total layer count $K$.
\end{proof}
...
\begin{tikzpicture}[
  ...
  % T2 est à droite de T1, mais on remonte de 0.8cm pour annuler la descente de T1
  \node[tensor, right=of T1, yshift=0.8cm] (T2) {T2 $[1,3]$};
  
  % T3 est en dessous de T1 (il descend donc naturellement avec lui)
...
We evaluate \textsc{NeuroDSL} on multiple fronts: (i) training speed and memory usage against Flux and PyTorch, (ii) effectiveness of the dynamic optimisations, (iii) multi-GPU scaling, (iv) end-to-end training on real datasets, and (v) micro-benchmarks to assess overhead. All experiments were conducted on a laptop equipped with an NVIDIA RTX A5500 (16~GB VRAM) and an Intel Core i9 CPU, running Julia~1.10 and PyTorch~2.5.1.

\subsection{Transformer Block Performance}
We compare the forward+backward time and peak GPU memory on a standard Transformer block \cite{vaswani} (RMSNorm, single-head attention, SwiGLU MLP) with sequence length 64 and varying hidden dimensions. For Flux and PyTorch, we use equivalent implementations; for \textsc{NeuroDSL}, we use the \texttt{full=true} backward mode (no incremental saving). Timings are medians over 50 samples after warm-up; memory is measured with \texttt{MemTrack} (Julia) and \texttt{torch.cuda.max\_memory\_allocated} (PyTorch).

\begin{table}[H]
\centering
\caption{Transformer block – time (ms) and peak memory (MiB) on GPU.}
\label{tab:transformer}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c c c c c c}
\toprule
\textbf{dim} & \textbf{Flux (ms)} & \textbf{ND (ms)} & \textbf{PyT (ms)} & \textbf{ND mem (MiB)} & \textbf{PT mem (MiB)} \\
\midrule
256  & 8.18 & 6.55 & 5.10 & 3.47  & 20.32 \\
512  & 8.42 & 6.09 & 5.14 & 10.35 & 31.38 \\
1024 & 8.44 & 5.96 & 5.77 & 34.61 & 74.51 \\
\bottomrule
\end{tabular}
\end{table}

\textsc{NeuroDSL} is 20--30\% faster than Flux and within 3\% of PyTorch at larger sizes, while consuming \textbf{2--6$\times$ less memory}. We note that at smaller operational scales, a substantial portion of PyTorch's reported memory footprint stems from fixed runtime initialization and caching overhead within the \texttt{THCCachingAllocator}. Nevertheless, \textsc{NeuroDSL}'s deterministic interval-coloring allocation completely avoids allocation fragmentation, providing a tight, provably optimal bound as the graph scales.
...
On a tiny model (10$\times$5 matrix multiply + sum), \textsc{NeuroDSL} completes the forward pass in \textbf{9.4~$\mu$s} and the backward pass in \textbf{21~$\mu$s}, with only 1.67~KiB allocated. This demonstrates that the dynamic engine imposes negligible overhead for small graphs. We also measured memory allocations for a linear model ($\text{Linear} \to \text{LayerNorm} \to \text{Linear}$) across sizes (see \Cref{tab:linear_mem}). Allocations scale linearly with tensor dimensions, and repeated runs show no memory leak (drift $<400$~KB after 10 iterations).
...
\midrule
128$\times$256 & 131.7 & 2315.3 & 2447.0 \\
512$\times$512 & 1027.8 & 14355.2 & 15383.0 \\
1024$\times$512 & 2051.8 & 24597.2 & 26649.0 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Dynamic Optimisations}
\paragraph{Incremental backward.} On a 10-layer network, modifying only the last parameter reduces the backward time by 24--89%, depending on which parameter is changed. \Cref{tab:incr_backward} shows the median time over 50 runs for a full backward versus the incremental one.

\begin{table}[H]
\centering
\caption{Incremental backward on a 10-layer MLP (dim=64).}
\label{tab:incr_backward}
\begin{tabular}{c c c}
\toprule
\textbf{Method} & \textbf{Time ($\mu$s)} & \textbf{Gain} \\
\midrule
Full recomputation & 44\,943.8 & -- \\
Incremental (last param) & 33\,999.6 & 24.4\% \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Operator fusion.} Fusing \texttt{matmul+relu} on a small network reduces forward time by \textbf{37.2\%} (CPU) and up to 53.8\% on GPU (measured on a simple chain).

\begin{table}[H]
\centering
\caption{Operator fusion speedup.}
\label{tab:fusion}
\begin{tabular}{c c c}
\toprule
\textbf{Device} & \textbf{Before ($\mu$s)} & \textbf{After ($\mu$s)} \\
\midrule
CPU & 261.6 & 164.4 (37.2\%) \\
GPU & 206.3 & 95.3 (53.8\%) \\
\bottomrule
\end{tabular}
\end{table}
...
As an architectural test of scaling correctness, we train a standard MLP ($784 \to 128 \to 10$) on MNIST using two GPUs. Each GPU processes half of the mini-batches independently, and weights are synchronised every 5 epochs to minimize communication bottlenecks. \Cref{tab:multigpu} summarises the results.

\begin{table}[H]
\centering
\caption{MNIST training on 1 vs 2 GPUs (20 epochs).}
\label{tab:multigpu}
\begin{tabular}{c c c}
\toprule
\textbf{Configuration} & \textbf{Total time (s)} & \textbf{Final accuracy} \\
\midrule
1$\times$ A5500 & 42.13 & 93.73\% \\
2$\times$ A5500 (sync/5 epochs) & 22.13 & 91.58\% \\
\bottomrule
\end{tabular}
\end{table}

The 2-GPU setup achieves a \textbf{1.9$\times$ speedup}. The minor accuracy degradation (from 93.73\% to 91.58\%) is an expected theoretical consequence of the loose, delayed weight synchronization window, which acts similarly to a Local SGD or a coarse Federated Averaging framework. This design explicitly trades immediate local convergence speed for massive communication throughput benefits. If absolute accuracy parity is requested by the topology, a standard synchronous all-reduce loop can be activated.
...
\midrule
Transformer speed (dim=1024) & 5.96 ms & 1.03$\times$ vs PyTorch, 1.42$\times$ vs Flux \\
Peak GPU memory (dim=1024)   & 34.61 MiB & 2.15$\times$ less than PyTorch \\
Iris test accuracy           & 100\%     & -- \\
Fusion speedup               & 37--54\%  & -- \\
Incremental backward gain     & 24--89\%  & -- \\
Reactive early stopping      & 97.2\%    & iteration reduction \\
Multi-GPU speedup (2 GPUs)   & 1.9$\times$ & vs single GPU \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Fusion coverage.} The current \texttt{FUSION\_TABLE} covers only two patterns (\texttt{matmul+relu} and \texttt{relu+matmul}). Extending it to attention kernels, layer normalization variants, and element-wise chains would unlock further performance gains. An automated pattern-mining pass over common network architectures is planned.
...
\paragraph{Gradient correctness at scale.} The incremental backward has been validated on up to 10-layer MLPs. Correctness proofs and empirical validation on large Transformer models (hundreds of layers, billions of parameters) remain as future work.
...
We have presented \textsc{NeuroDSL}, a novel deep learning framework centred on a mutable computational graph. Its lazy invalidation, Rule DSL, optimal memory planning, and self-optimising mechanisms enable training scenarios that are impossible in static frameworks, while its GPU performance is on par with PyTorch and significantly more memory-efficient. The multi-GPU extension further demonstrates the flexibility of the approach, and the interactive visualization provides a powerful tool for debugging and education. \textsc{NeuroDSL} achieves 100\% test accuracy on Iris, effectively learns complex non-linear functions, and demonstrates 2--6$\times$ memory reduction against PyTorch on standard Transformer benchmarks. Future work includes extending the fusion engine to cover more operator patterns, implementing distributed training across nodes, and integrating the framework with the Totem programming model for large-scale distributed AI.
