...
function cross_entropy_grad(logits::AbstractMatrix{Float32}, labels::AbstractVector{<:Integer})
    if Backend.CUDA_AVAILABLE && logits isa CUDA.CuArray
        ...
        g = copy(p)
        n = size(logits, 1)
        labels_gpu = CUDA.cu(Int32.(collect(labels)))
        threads = min(256, n)
        blocks  = cld(n, threads)
...
#mse_loss_bwd(out, target, dy) = (2f0 ./ length(out)) .* (out .- target) .* sum(dy)
mse_loss_bwd(out, target, dy) = (2f0 ./ length(out)) .* (out .- target) .* dy
# ── Sum of matrix elements (identique) ─────────────────────────────────
sum_matrix_fwd(x) = [sum(x)]
sum_matrix_bwd(dev, x_val, dy) = Backend.ones32(dev, size(x_val)...) .* sum(dy)

# ── Forward embedding CUDA kernel (identique) ──────────────────────────
if Backend.CUDA_AVAILABLE
