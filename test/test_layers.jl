...
@testset "Layers" begin
    ...
    @testset "LayerNorm shape" begin
        g = NeuroDSL.NeuroGraph(namespace=:t_ln, device=dev)
    ...
    @testset "Linear shape" begin
        g = NeuroDSL.NeuroGraph(namespace=:t_li, device=dev)
    ...
    @testset "LlamaBlock residual shape" begin
        g = NeuroDSL.NeuroGraph(namespace=:t_lb, device=dev)
        NeuroDSL.set!(g,:x, randn(Float32,4,16))
    ...
    @testset "LlamaModel 2 layers" begin
        g = NeuroDSL.NeuroGraph(namespace=:t_lm, device=dev)
        NeuroDSL.set!(g,:x, randn(Float32,4,16))
