...
push!(checkpoints, Symbol(mha1,:_dec_concat));                       push!(labels, "hcat_heads SEUL (couche 1)")
push!(checkpoints, Symbol(mha1,:_dec_output));                       push!(labels, "projection sortie SEULE (couche 1)")
...
layer1_attn_ops = sum(med_by_cp[4:10])   # slice+rope Q, slice+rope+cache K/V, QK, scale+softmax, PV, hcat(seul), outproj(seul)
layer1_total = sum(med_by_cp[3:11])
println("-"^86)
...
hcat_only = med_by_cp[9]; outproj_only = med_by_cp[10]
...
println("  SPLIT hcat_heads (12 kernels) vs projection sortie (1 matmul)        :")
println("    hcat_heads seul   : $(round(1000*hcat_only,digits=3))ms")
println("    projection seule  : $(round(1000*outproj_only,digits=3))ms")

open(joinpath(@__DIR__, "diag_decode_hcat_split_results.json"), "w") do io
    JSON.print(io, Dict(
        ...
        "hcat_only_median_ms"=>1000*hcat_only, "outproj_only_median_ms"=>1000*outproj_only,
    ), 2)
end
_log("Écrit -> diag_decode_hcat_split_results.json")

# ── Sonde supplémentaire (ajoutée après coup) : le coût de "hcat_heads SEUL"
# vient-il de `_ancestors_of!` (première fois que ce symbole précis est CIBLE
# directe d'un `demand!`) ou de `execute_rule!` (calcul réel) -- même
# méthode que la sonde "embedding" plus haut, appliquée au nœud suspect
# après un prototype de kernel fusionné qui n'a montré AUCUN gain en
# situation réelle malgré un gain net mesuré en isolation (~4.6x sur un
# micro-benchmark hors graphe). ─────────────────────────────────────────────
_log("Sonde bis : _ancestors_of!(concat_sym) vs execute_rule! réel, séparément...")
concat_sym = Symbol(mha1,:_dec_concat)
NeuroDSL.set!(g, :dec_token_id, [3]; atom_type=NeuroDSL.Datom, namespace=ns)
NeuroDSL.set!(g, :dec_cur_step, Float32[cur_step+1]; namespace=ns)
NeuroDSL.set!(g, :dec_pos, Float32[cur_step]; namespace=ns)
NeuroDSL.invalidate_all!(g; namespace=ns)
NeuroDSL.demand!(g, Symbol(mha1,:_dec_ao_h,N_HEADS); namespace=ns)  # amène tout en amont à valid=true
CUDA.synchronize()
t_anc2 = @elapsed anc_concat = NeuroDSL._ancestors_of!(g, ns, concat_sym)
_log("  _ancestors_of!(concat_sym) [1re fois CIBLE directe] : $(round(1000*t_anc2,digits=3))ms, longueur=$(length(anc_concat))")
t_anc2b = @elapsed anc_concat2 = NeuroDSL._ancestors_of!(g, ns, concat_sym)  # devrait être un HIT
_log("  _ancestors_of!(concat_sym) [2e appel, cache DEVRAIT être un hit] : $(round(1000*t_anc2b,digits=3))ms")
CUDA.synchronize()
t_exec_only = @elapsed NeuroDSL.demand!(g, concat_sym; namespace=ns)
CUDA.synchronize()
_log("  demand!(concat_sym) mur (ancêtres déjà en cache + execute_rule! réel) : $(round(1000*t_exec_only,digits=3))ms")
# Un 2e demand! sur le MÊME symbole, sans invalidation -- devrait être quasi
# instantané (valid=true déjà), confirme que le coût n'est pas dans
# execute_rule! lui-même s'il est bas ici aussi.
t_exec_again = @elapsed NeuroDSL.demand!(g, concat_sym; namespace=ns)
_log("  demand!(concat_sym) 2e appel SANS invalidation (devrait ~0) : $(round(1000*t_exec_again,digits=3))ms")
